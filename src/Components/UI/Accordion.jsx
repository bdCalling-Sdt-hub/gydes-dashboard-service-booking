...
const Accordion = ({
  ...
  return (
    <div
      className={cn(
        "mb-5 bg-[#0861C522] duration-500 rounded shadow ",
        className
      )}
    >
      <div className="flex justify-between items-center p-4 cursor-pointer  duration-500">
        ...
        <div className="flex gap-2">
          <div onClick={() => showFaqUpdateModal(item)} className="p-[2px] ">
            <FaEdit className="text-secondary-color text-base md:text-lg lg:text-xl duration-500" />
          </div>
          <div onClick={() => showFaqDeleteModal(item)} className="p-[2px] ">
            <MdDelete className="text-secondary-color text-base md:text-lg lg:text-xl duration-500" />
          </div>
          {isOpen ? (
            <div
      ...
      >
        <div className="p-4 bg-[#0861C511] text-base-color duration-500 text-sm md:text-base lg:text-lg rounded-bl rounded-br">
          {item?.answer}
        </div>
      </div>
    </div>
